...
PLUMED_REGISTER_ACTION(GlobalDistortion,"GLOBALDISTORTIONOP")
...
void GlobalDistortion::registerKeywords(Keywords& keys) {
  ...
  keys.addOutputComponent("omp_threads","default","number of OpenMP threads detected by OpenMP::getNumThreads()");
}
  ...
  pbc(true)
{
  ...
  addComponent("omp_threads"); componentIsNotPeriodic("omp_threads");
  getPntrToComponent("omp_threads")->set(static_cast<double>(OpenMP::getNumThreads()));
...
void GlobalDistortion::calculate() {
  ...
  getPntrToComponent("omp_threads")->set(static_cast<double>(nthreads));
  double pairsum = 0.0;
  double prefactor;
  double value;
