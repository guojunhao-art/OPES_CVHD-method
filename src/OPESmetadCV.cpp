...
//+PLUMEDOC OPES_BIAS OPES_METADCV
/*
...
This \ref OPES_METADCV action samples target distributions defined via their marginal \f$p^{\text{tg}}(\mathbf{s})\f$ over some collective variables (CVs), \f$\mathbf{s}=\mathbf{s}(\mathbf{x})\f$.
By default \ref OPES_METADCV targets the well-tempered distribution, \f$p^{\text{WT}}(\mathbf{s})\propto [P(\mathbf{s})]^{1/\gamma}\f$, where \f$\gamma\f$ is known as BIASFACTOR.
Similarly to \ref METAD, \ref OPES_METADCV optimizes the bias on-the-fly, with a given PACE.
...
As an intuitive picture, rather than gradually filling the metastable basins, \ref OPES_METADCV quickly tries to get a coarse idea of the full free energy surface (FES), and then slowly refines its details.
...
Notice that \ref OPES_METADCV is more sensitive to degenerate CVs than \ref METAD.
If the employed CVs map different metastable basins onto the same CV-space region, then \ref OPES_METADCV will remain stuck rather than completely reshaping the bias.
This can be useful to diagnose problems with your collective variable.
If it is not possible to improve the set of CVs and remove this degeneracy, then you might instead consider to use \ref OPES_METAD_EXPLORECV or \ref METAD.
...
On the contrary, if your CVs are not degenerate but only suboptimal, you should converge faster by using \ref OPES_METADCV instead of \ref METAD \cite Invernizzi2020rethinking.
...
opes: OPES_METADCV ARG=cv PACE=200 BARRIER=40
...
template <class mode>
class OPESmetadCV : public bias::Bias {
...
public:
  explicit OPESmetadCV(const ActionOptions&);
...
typedef OPESmetadCV<convergence> OPESmetadCV_c;
PLUMED_REGISTER_ACTION(OPESmetadCV_c,"OPES_METADCV")

//OPES_METAD_EXPLORECV is very similar from the point of view of the code,
//but conceptually it is better to make it a separate BIAS action

//+PLUMEDOC OPES_BIAS OPES_METAD_EXPLORECV
/*
...
This \ref OPES_METAD_EXPLORECV action samples the well-tempered target distribution, that is defined via its marginal \f$p^{\text{WT}}(\mathbf{s})\propto [P(\mathbf{s})]^{1/\gamma}\f$ over some collective variables (CVs), \f$\mathbf{s}=\mathbf{s}(\mathbf{x})\f$.
While \ref OPES_METADCV does so by estimating the unbiased distribution \f$P(\mathbf{s})\f$, \ref OPES_METAD_EXPLORECV instead estimates on-the-fly the target \f$p^{\text{WT}}(\mathbf{s})\f$ and uses it to define the bias.
The bias at step \f$n\f$ is
\f[
...
Intuitively, while \ref OPES_METADCV aims at quickly converging the reweighted free energy, \ref OPES_METAD_EXPLORECV aims at quickly sampling the target well-tempered distribution.
...
Compared to \ref OPES_METADCV, \ref OPES_METAD_EXPLORECV is more similar to \ref METAD, because it allows the bias to vary significantly, thus enhancing exploration.
This goes at the expenses of a typically slower convergence of the reweight estimate.
\ref OPES_METAD_EXPLORECV can be useful e.g.~for simulating a new system with an unknown BARRIER, or for quickly testing the effectiveness of a new CV that might be degenerate.

Similarly to \ref OPES_METADCV, also \ref OPES_METAD_EXPLORECV uses a kernel density estimation with the same on-the-fly compression algorithm.
...
All the options of \ref OPES_METADCV are also available in \ref OPES_METAD_EXPLORECV, except for those that modify the target distribution, since only a well-tempered target is allowed in this case.
...
\plumedfile
cv: DISTANCE ATOMS=1,2
opes: OPES_METAD_EXPLORECV ARG=cv PACE=500 BARRIER=40
...
typedef OPESmetadCV<exploration> OPESmetadCV_e;
// For some reason, this is not seen correctly by cppcheck
// cppcheck-suppress unknownMacro
PLUMED_REGISTER_ACTION(OPESmetadCV_e,"OPES_METAD_EXPLORECV")

template <class mode>
void OPESmetadCV<mode>::registerKeywords(Keywords& keys)
{
...
//extra options
  ...
  keys.add("optional","SIGMA_MAX","never reduce SIGMA below this value");
...
template <class mode>
OPESmetadCV<mode>::OPESmetadCV(const ActionOptions& ao)
  : PLUMED_BIAS_INIT(ao)
  , isFirstStep_(true)
  , counter_(1)
  , mycounter_(1)
...
//other compulsory input
  ...
  if (cvhd)
  {
    ...
    getPntrToComponent("slo")->set(0.0);
    addComponent("posibias"); componentIsNotPeriodic("posibias");
    getPntrToComponent("posibias")->set(0.0);
  }
  if(cvhd)
  {
    cutoff=4.0;
...
//add and set output components
  ...
  addComponent("myrct");
  componentIsNotPeriodic("myrct");
  getPntrToComponent("myrct")->set(kbt_*std::log(sum_weights_/counter_));
...
//printing some info
  ...
  if(monitor_.size()>0)
  {
    log.printf(" -- The collective variable for reaction filter: ");
    for(unsigned a=0; a< monitor_.size(); a++)
      log.printf("%s (upper: %g) ",monitor_[a]->getName().c_str(), cvfiltermax_[a]);
    log.printf("will be used\n");
  }
  if(adaptive_sigma_)
  {
...
template <class mode>
void OPESmetadCV<mode>::calculate()
{
//get cv
...
template <class mode>
void OPESmetadCV<mode>::update()
{
  bool isMax = false;
  if(cvhd) 
  {
    bool clean_bias = false;
    for(unsigned i=0; i<ncv_; ++i) {
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if(getStep()%stride_==0 && (excluded_region_==NULL || excluded_region_->get()==0))
  {
    ...
    /////////////////////////////////////////////////////////////////////////
    {
      ...
      if(cvhd && kernels_.size()!=0)
      {
        bool nowadd=true;
        for(unsigned i=0; i<ncv_; i++)
        {
          if((1.0-center[i])/sigma[i]<=4.0)
          {
            nowadd=false;
            break;
          }
      ...
      else
      {
        if(cvhd & kernels_.size()==0)
        {
          ...
          {
            double sig=sigma[i];
            sigma[i]=std::min(sigma[i],(1.0-center[i])/(5.0));
            height*=(sig/sigma[i]);
          }
        }
        addKernel(height,center,sigma,log_weight);
...
template <class mode>
double OPESmetadCV<mode>::getProbAndDerivatives(const std::vector<double>& cv,std::vector<double>& der_prob)
{
  double prob=0.0;
  if(!nlist_)
  {
...
template <class mode>
void OPESmetadCV<mode>::addKernel(const double height,const std::vector<double>& center,const std::vector<double>& sigma)
{
  bool no_match=true;
  if(threshold2_!=0)
  {
...
template <class mode>
void OPESmetadCV<mode>::addKernel(const double height,const std::vector<double>& center,const std::vector<double>& sigma,const double logweight)
{
  addKernel(height,center,sigma);
//write to file
...
template <class mode>
unsigned OPESmetadCV<mode>::getMergeableKernel(const std::vector<double>& giver_center,const unsigned giver_k)
...
template <class mode>
void OPESmetadCV<mode>::updateNlist(const std::vector<double>& new_center)
{
...
template <class mode>
void OPESmetadCV<mode>::dumpStateToFile()
{
...
template <class mode>
inline double OPESmetadCV<mode>::evaluateKernel(const kernel& G,const std::vector<double>& x) const
...
template <class mode>
inline double OPESmetadCV<mode>::evaluateKernel(const kernel& G,const std::vector<double>& x, std::vector<double>& acc_der, std::vector<double>& dist)
{ //NB: cannot be a method of kernel class, because uses external variables (for cutoff)
  double norm2=0;
  for(unsigned i=0; i<ncv_; i++)
  {
...
template <class mode>
inline void OPESmetadCV<mode>::mergeKernels(kernel& k1,const kernel& k2)
{
...
template <class mode>
double OPESmetadCV<mode>::evaluate_slope(std::vector<double>& raw)
{
  unsigned len=raw.size();
  double sx,sxx,sy,sxy,value;
  sx=0;
  sy=0;
  sxx=sxy=0;
  value=0;
  for(unsigned i=0; i<len; i++)
  {
   sx = sx + (double)i;
   sy = sy + raw[i];
   sxx = sxx + (double)i * (double)i;
   sxy = sxy + (double)i * raw[i];
  }
  double numerator = len*sxy - sx*sy;
  double denominator = len*sxx - sx*sx;
...
template <class mode>
double OPESmetadCV<mode>::calculate_maximum(const std::vector<double>& cv)
{
  std::vector<double> der_prob(ncv_,0);
  std::vector<double> point_(2,0);
  double largest = 0;
  for(unsigned i=0; i<20; i++)
  {
    point_[0]=cv[0];
    point_[1]=0.05*static_cast<double>(i);
    const double prob=getProbAndDerivatives(point_,der_prob);
    const double vbias=kbt_*bias_prefactor_*std::log(prob/Zed_+epsilon_)+barrier;
    largest=std::max(largest, vbias);
  }

  for(unsigned i=0; i<20; i++)
  {
    point_[1]=cv[1];
    point_[0]=0.05*static_cast<double>(i);
    const double prob=getProbAndDerivatives(point_,der_prob);
    const double vbias=kbt_*bias_prefactor_*std::log(prob/Zed_+epsilon_)+barrier;
    largest=std::max(largest, vbias);
  }
